...
export class DataSinkService {
    ...
    get dataSinks() { 
        return this._store.state.pipe(filter(x => x.status != Status.NotInitialized)).pipe(map(x => x.dataSinks));
    }
    ...
    async getDataSinks() {
        ...
        try {
            const dataSinks = await this.httpService.get<api.DataSinkType[]>(`/datasinks`, undefined, DATA_SINKS_MOCK());
            this._store.patchState(state => {
    ...
    async getDataSink(datasourceId: string) {
        ...
        try {
            const obj = await this.httpService.get(`/datasinks/${datasourceId}`, undefined, DATA_SINKS_MOCK()[0]);
            this._store.patchState(state => {
                state.status = Status.Ready;
