...
function dataPointGetHandler (request: Request, response: Response) {
    const sink = configManager?.config?.dataSinks.find((sink) => sink.id === request.params.datasinkId);
    const point = sink.dataPoints.find((datapoint) => datapoint.id === request.params.dataPointId)
...
function dataPointsPostHandler (request: Request, response: Response) {
    ...
    const changedSinkObject = configManager.config.dataSinks.find((sink) => sink.id === request.params.datasinkId);
...
function dataPointPatchHandler (request: Request, response: Response) {
    ...
    const sink = configManager?.config?.dataSinks.find((sink) => sink.id === request.params.datasinkId);
    const index = sink.dataPoints.findIndex((point) => point.id === request.params.dataPointId);
