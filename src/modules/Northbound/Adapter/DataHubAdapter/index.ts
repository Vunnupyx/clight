...
export class DataHubAdapter {
  ...
  public setReportedProps(reportedServices: string[]): void {
    ...
    const removedServices = this.#alreadyReportedServices.filter(s => !reportedServices.includes(s));
    const newServices = reportedServices.filter(s => !this.#alreadyReportedServices.includes(s));
    ...
    return Promise.resolve()
      ...
      .catch((err) => {
        return Promise.reject(
          new NorthBoundError(
            `${logPrefix} error due to ${err.message}`
          )
        );
      });
  }
