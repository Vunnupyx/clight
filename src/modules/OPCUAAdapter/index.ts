...
export class OPCUAAdapter {
    ...
    private static configValidation(config: ConfigManager) {
        ...
        if(!config) throw new NorthBoundError(`${logPrefix} no ConfigManager found.`);
        if(!config.runtimeConfig) throw new NorthBoundError(`${logPrefix} no runtime config found.`);
        if(!config.runtimeConfig.opcua) throw new NorthBoundError(`${logPrefix} no opcua config found.`);
    }
    ...
    public async init(): Promise<OPCUAAdapter> {
        ...
        if(this.server.initialized) {
            winston.info(`${OPCUAAdapter.className}::init adapter already initialized.`)
            return this;
        }
        return this.server.initialize()
            ...
            return Promise.reject(new AdapterError(`${logPrefix} error during initial node-opcua server due to ${JSON.stringify(err)}.`));
        });
    }
    ...
    public async stop(timeoutMs?: number): Promise<void> {
        ...
        return this.server.shutdown(timeoutMs || OPCUAAdapter.shutdownTimeoutMs)
            .then(() => {
                this.running = false;
                winston.info(`${logPrefix} adapter stopped.`);
            })
            .catch((err) => {
                const errMsg = `${logPrefix} try to stop adapter error due to ${JSON.stringify(err)}`;
    ...
    private initCheck(logPrefix: string): AdapterError | void {
        if (!this.server.initialized) {
            return new AdapterError(`${logPrefix} error due to adapter not initialized.`);
        }
    }
}